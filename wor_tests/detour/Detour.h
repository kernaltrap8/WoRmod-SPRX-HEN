...
public:
	...
	)
	{
		...
		const UINT32 *BranchAsm     = PreserveRegister ? BranchFarAsmPreserve : BranchFarAsm;
		const UINT32 BranchAsmSize = PreserveRegister ? sizeof( BranchFarAsmPreserve ) : sizeof( BranchFarAsm );
	...
	)
	{
		...
		const UINT32 InstructionAddress = (UINT32)Source;
		...
		const void* BranchAddress = (void*)(INT32)( InstructionAddress + BranchOffset );
	...
	bool Install()
	{
		...
		const SIZE_T HookSize = Detour::WriteFarBranch( NULL, this->HookTarget, false, false );
		...
		{
			const UINT32* InstructionPtr = (UINT32*)( ( UINT32 )this->HookSource + ( i * 4 ) );
		...
		//
		const void* AfterBranchAddress = (void*)( ( UINT32 )this->HookSource + HookSize );
	...
	static UINT32 *LocalExecutableScratchpad;
private:
